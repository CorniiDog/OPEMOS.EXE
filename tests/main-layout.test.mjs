...
test("manifest-bound NVIDIA outputs skip rebuilding and become USB-ready", () => {
  ...
  assert.match(script, /nvidiaSource\.addEventListener\("change", \(\) => \{[\s\S]*!admitBuildSourceSelection\(currentBuildSnapshot\(\)\)\.accepted[\s\S]*nvidiaSource\.value = acceptedNvidiaSource;[\s\S]*allowUpstreamBuild\.checked = acceptedUpstreamApproval;[\s\S]*acceptedNvidiaSource = elements\.nvidiaSource\.value;/);
