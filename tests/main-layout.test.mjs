...
test("manifest-bound NVIDIA outputs skip rebuilding and become USB-ready", () => {
  ...
  assert.match(script, /admitBuildStart,[\s\S]*admitBuildSourceRefresh,[\s\S]*admitBuildSourceSelection,[\s\S]*admitImageSelection,[\s\S]*admitExportModeSelection,[\s\S]*admitOutputDirectorySelection,[\s\S]*admitUsbPreflightCancel,[\s\S]*admitUsbConfirmationEdit,[\s\S]*admitUsbPreflightStart,[\s\S]*admitUsbReviewOpen,[\s\S]*admitUsbReviewDismiss,[\s\S]*admitUsbTargetSelection,[\s\S]*admitUsbTargetRefresh,[\s\S]*admitUsbTargetClear,[\s\S]*deriveBuildAdmission,[\s\S]*from "\.\/workflow-state\.js"/);
  ...
  assert.match(script, /async function loadNvidiaSourceBranches\(\)[\s\S]*generation = \+\+sourceBranchLoadGeneration;[\s\S]*await invoke\("list_nvidia_source_branches"\)[\s\S]*admitBuildSourceRefresh\(currentBuildSnapshot\(\), \{[\s\S]*generation,[\s\S]*currentGeneration: sourceBranchLoadGeneration,[\s\S]*\}\)\.accepted\) return;[\s\S]*querySelectorAll\("optgroup"\)[\s\S]*generation !== sourceBranchLoadGeneration\) return;/);
  assert.match(script, /async function loadNvidiaSourceBranches\(\) \{[\s\S]*if \(!admitBuildSourceSelection\(currentBuildSnapshot\(\)\)\.accepted\) return;[\s\S]*await invoke\("list_nvidia_source_branches"\)/);
  assert.match(script, /catch \(error\) \{[\s\S]*admitBuildSourceRefresh\(currentBuildSnapshot\(\), \{[\s\S]*generation,[\s\S]*currentGeneration: sourceBranchLoadGeneration,[\s\S]*\}\)\.accepted[\s\S]*Could not load optional NVIDIA branches/);
