...
test("manifest-bound NVIDIA outputs skip rebuilding and become USB-ready", () => {
  ...
  assert.match(script, /const sourceBranchLoadGate = createLatestRequestGate\(\)/);
  assert.match(script, /function sourceBranchRefreshCapability\(generation\)[\s\S]*sourceBranchLoadGate\.isCurrent\(generation\)[\s\S]*generation === 1 \? 2 : 1/);
  assert.match(script, /async function loadNvidiaSourceBranches\(\)[\s\S]*generation = sourceBranchLoadGate\.begin\(\);[\s\S]*await invoke\("list_nvidia_source_branches"\)[\s\S]*admitBuildSourceRefresh\(currentBuildSnapshot\(\), sourceBranchRefreshCapability\(generation\)\)\.accepted\) return;[\s\S]*querySelectorAll\("optgroup"\)[\s\S]*!sourceBranchLoadGate\.isCurrent\(generation\)\) return;/);
  ...
  assert.match(script, /catch \(error\) \{[\s\S]*admitBuildSourceRefresh\(currentBuildSnapshot\(\), sourceBranchRefreshCapability\(generation\)\)\.accepted[\s\S]*Could not load optional NVIDIA branches/);
