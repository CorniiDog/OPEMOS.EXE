...
The answer file automates Windows Setup only for contained disk 0: it creates one 100 MiB EFI partition, one 16 MiB MSR partition, and one NTFS Windows partition using the remaining space. The generator refuses extra disks, added or reordered partition actions, mismatched modification targets, and any other install target before creating output. Setup shows its UI on error and does not target any second disk.
