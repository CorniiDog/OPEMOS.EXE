...
- [ ] Deliver consistent Windows, Linux, and macOS self-contained bundle entry points. The first slice added compiled manifest-SHA binding, bundle-relative command resolution without packaged `PATH` fallback, closed schema-1 staging/consumer validation, and consistent entry points. Native Linux and Windows clean-environment startup and representative bundled-command execution passed with exact payload/application/provenance identities. Core approved exact PR #96 base `d21410a21ba9db84b99c4c45118d5a77484bb99c` and head `47fe5b5dea25fffca355beb7666b2057d9915365` after all required checks passed; protected main squash-merged it as `b3dbc965626cf4d1818de69791afc3cc73e8ea4c`. The active acquisition follow-up pins the proven 62-archive Ubuntu 24.04 x86_64 closure by version, size, SHA-256, and license, verifies or safely replaces cache entries, stages through a same-parent temporary directory, retains optional `--runtime-root` offline input, and acquires automatically when omitted. Focused Python passes 8/8; Python and Bash syntax and diff hygiene pass. Native construction from the authenticated archive cache validates exact manifest SHA-256 `b7841c4296b074fcac033cf03ee75ec9ff56c2ca4d06f58e130c413af5899ee1`, clean-environment Git `2.43.0`, QEMU img `8.2.2`, Python `3.12.3`, transactional cleanup, and idempotent reuse. Windows acquisition, honest macOS pinned recipe/static-contract validation, broader checks, exact-head review, PR, and merge remain pending.
