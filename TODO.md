...
- [ ] Add the network-free Windows `partial` orchestration harness. It must reject mismatched independent executable, official SteamOS, immutable driver-only bundle, and exact harness-owned 32 GiB virtual-USB identities before action work; run owned construct/export/enumerate/write/flush/readback phases and cleanup under one total deadline; and emit only validated construction/write/readback evidence with explicit no-install and no-publication claims. This bounded source work performs no download, VM launch, image construction, media write, install, release, or publication.
