...
* [x] Immutably pin the support repository's offline installer, reviewed userspace lock, and minimal signer keyring for the normal workflow: exact support commit, ten required paths, byte counts, SHA-256 hashes, safe staging, cancellation cleanup, and a versioned bundle manifest are enforced without accepting a user-selected checkout or moving branch.
...
* [ ] Require the support measurer to preserve a bounded structured failure phase, command identity, exit status, and stderr summary instead of collapsing missing tools, loop/mount failures, parsing failures, and ENOSPC into `compression_measurement_failed`.
...
* [x] Define and consume strict schema-1 `STEAMOS_NVIDIA_PROGRESS` JSON lines for detailed offline-root validation phases, real byte/item ratios, bounded attempts, readable phase logs, and app-owned status wording. The pinned support validator still needs to emit this contract before live progress appears.
...
* [ ] Define CUDA omission in the NVIDIA support repository as a versioned, reviewed, package-owned gaming payload profile. Preserve graphics, Vulkan, GLVND/EGL, NVENC/NVDEC, GSP firmware, required 32-bit gaming libraries, recovery rendering, provenance, and pacman consistency; never implement it as builder-owned filename deletion.
* [ ] Add a canonical support-repository repack command for already-published raw-module artifacts: authenticate the original archive/checksum/provenance, deterministically encode identical module payloads as `.ko.zst`, regenerate representation hashes and provenance, and publish a create-only revision tag/assets without overwriting the original release. Let the image builder offer this only after validating the support-owned dry-run contract.
