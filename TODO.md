...
- [ ] Deliver consistent Windows, Linux, and macOS self-contained bundle entry points. The current branch adds a compiled manifest-SHA binding, bundle-relative command resolution without packaged `PATH` fallback, and closed staging validation for the exact platform command inventory, regular-file sizes/hashes, component versions, and declared license files. It also adds `bundle_windows.ps1`, `bundle_linux.sh`, and `bundle_macos.sh` with consistent runtime-root inputs and `dist/<platform>` outputs. After Core identified that the initial Rust consumer omitted the producer-required `components` field, the single schema-1 contract now enforces the same bounded unique component names, versions, and declared nonempty license lists on both sides. A cross-language regression stages a real-shaped manifest with the Python producer and successfully verifies it through the compiled Rust consumer; malformed, duplicate, empty, and undeclared component/license cases fail closed. Focused Rust passes 4/4 and Python passes 5/5 locally through `heavy.sh`; native Linux and Windows clean-environment package/startup plus representative bundled-command invocation, honest macOS contract validation, exact real payload identities/licenses, broader checks, fresh exact-head review, and PR evidence remain pending.
