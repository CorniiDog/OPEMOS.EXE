...
* [ ] Replace incremental latest-package dependency discovery with a versioned, support-repository-owned userspace lock manifest for each supported NVIDIA release: resolve the complete transitive closure before an end-user build, pin exact filenames/versions/hashes/signatures/package-specific signer fingerprints, authenticate every signer against reviewed Arch key material, and require maintainer review rather than expanding trust at runtime.
* [ ] Make normal validation consume the complete locked dependency set in one bounded handoff; report an unknown or changed package/signer as a maintainer compatibility gate instead of making users repeat builds one dependency at a time.
