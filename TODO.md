...
* [ ] Add an installed **Open OPEMOS** welcome/recovery application, delivered by the support repository's canonical one-line installer as well as generated media. Switch or offer to switch into Desktop Mode before opening its graphical surface; show exact slot/kernel/NVIDIA health, fallback state, connectivity, release discovery, rebuild/install/verification progress, rollback, and storage actions through the support-owned machine-readable contract.
* [ ] Keep the Open OPEMOS frontend unprivileged. Pass only bounded, enumerated actions and revalidated device/slot identity documents to the privileged support helper; never regex-rewrite a device name, slot, command, or user input into a shell script.
* [ ] Reuse the OPEMOS frosted-glass component language in the installed application with an opaque dark fallback where compositor transparency is unavailable. Keep console/TTY status usable when Desktop Mode or graphics cannot start.
