...
* [ ] Extend the support resolver/installer contract to return and accept the complete missing signed Arch dependency package set (beginning with `egl-wayland`), including exact filenames, versions, hashes, signatures, reviewed signer identities, declared installed sizes, and recursive dependency/provides metadata.
* [ ] Generalize the Rust userspace handoff and result validation from exactly two NVIDIA packages to two required NVIDIA packages plus the resolver-owned dependency set; transfer every package/signature explicitly, require the same validated ordered manifest at mutation, and never let target pacman fetch an unpinned dependency from the network.
