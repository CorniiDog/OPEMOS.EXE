...
* [x] Immutably pin the support repository's offline installer, reviewed userspace lock, and minimal signer keyring for the normal workflow: exact support commit, sixteen required paths, byte counts, SHA-256 hashes, safe staging, cancellation cleanup, and a versioned bundle manifest are enforced without accepting a user-selected checkout or moving branch.
