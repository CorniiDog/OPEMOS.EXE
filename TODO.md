...
* [ ] Pin the reviewed support installer after its internal Holo database change lands; require its structured result to report `/usr/lib/holo/pacmandb` and a bounded nonzero package count.
...
* [ ] Update the pinned support installer to require the recovery EFI partition at `<root>/efi` while leaving rootfs `/boot` visible; validate both mount identities and add the required NVIDIA kernel command-line policy to `EFI/steamos/grub.cfg` without duplicating arguments on repeat runs.
