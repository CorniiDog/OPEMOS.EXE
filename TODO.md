...
- [ ] Consume canonical Core squash `30e2550895245729314a6ab91cc1753d5fbc7c15`
  ...
  tree `0d4d47ad35a155fa0b647fd8417301ab9620be47` validates the closed product
  ...
  SHA-256 `bb3cfeb5dda706e89a481e19dc5c811eb2b43adfe90962b1821f804784ec2ead`.
  ...
  The missing-target correction performs the read-only space preflight against
  the nearest existing ancestor when the kernel `updates` parent and target
  directory are absent. Its real cached-repair regression begins from that
  exact driver-absent layout and restores all five valve24.5 modules without
  curl or online installation.
  ...
  710,107 bytes. Formatting and warnings-denied all-target Clippy pass through
...
- [ ] Deliver the native Windows whole-removable-disk writer required by the
  2026-09-21 prerelease target. The implementation batch on
  `work/windows-native-usb-writer` reuses the cancellable write/flush/SHA-256
  readback engine, replaces read-only CIM discovery with guarded `Get-Disk`
  identity, rejects non-USB, boot, system, read-only, undersized, and unsupported-
  sector targets, requires an elevated Windows process, takes only the exact
  revalidated target offline, opens its canonical `PHYSICALDRIVE` exclusively
  with write-through, and binds the opened raw handle back to the whole-disk
  number and capacity. Completed-output transition now revalidates a visible
  selection against the final image, and the selector is a full-span build-card
  row. Local validation through the required heavy wrapper passes formatting,
