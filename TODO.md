...
- [ ] Enforce explicit machine-readable Windows imaging modes before adding VM execution. The network-free contract distinguishes `short` bounded feedback, `partial` official-SteamOS plus immutable compiled-driver-only bundle construction/export/write/flush/readback, and `full` source-evidence plus boot/install/reinstall/no-orphan publication gating. It requires sealed-base disposable overlays, exact executable and bundle identities, every mode-specific evidence bit, rejects shorter substitution and combined NVIDIA-plus-SteamOS assets, and never treats validation as publication authority. Core rejected PR #75 head `d369689043a9d472ead98bf0edda2da425a24381` because the result could select its own required mode, omitted negative declarations passed, shorter modes could carry contradictory boot/install claims, and non-short bundle evidence was not fully bound. The remediation requires an independent mode, explicit false reinstall/combined-asset/publication declarations, mode-correct boot/install claims, and canonical bundle schema, Release, ordered asset byte/hash, Core/source, target/compatibility, provenance, and build-source identities. Focused tests cover every omitted evidence field and identity, altered hashes/sizes/schemas, malformed identities, contradictory claims, overlay/base drift, and attempted publication; native execution remains a later contained-VM slice. This host has no Node runtime, so remote-only JavaScript validation requires a normal fast-forward update to `https://github.com/CorniiDog/steamos-nvidia-image-builder.git`, branch `work/exe-windows-imaging-modes`; no release, asset, image, or VM action is included.
