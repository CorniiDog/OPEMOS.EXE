...
* [x] Replace the normal Zenity welcome surface with the full-screen OPEMOS frosted-glass UI while retaining the same helper protocol and an opaque/terminal-safe Zenity fallback. Serve it only on a random loopback port with a private session token, exact-Origin API checks, a root-owned static bundle, and a close guard during active mutation.
