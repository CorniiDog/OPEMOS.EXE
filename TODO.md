...
* [ ] Repin the support implementation that applies and restores `compress-force=zstd:3` during mutation, then independently verify final package contents, modules, initramfs, Btrfs policy, cancellation cleanup, repeat execution, and the remaining free-space margin before exporting.
