...
* [x] Require the recovery root's Holo pacman database and reject the currently pinned support installer before dependency setup or mutation while it still hard-codes `/var/lib/pacman`; never initialize an empty database as a workaround.
* [ ] Extend the pinned support installer with an explicit, validated target pacman database contract for `/usr/lib/holo/pacmandb`; prove package ownership, rollback, repeat-run behavior, and compatibility with Valve's clone-based recovery/install flow before enabling mutation in the builder.
