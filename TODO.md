...
- [ ] Add the executable network-free Windows `short` runner. It must run the bounded unit/contract/UI, USB enumeration, physical-disk refusal, immutable driver-bundle manifest, and small-writer checks under one absolute deadline, always run owned cancellation cleanup, require exact executable identity, and emit only the validated non-E2E short result. Cover missing/false/throwing actions, cancellation, timeout, cleanup failure, and invalid candidate identity without VM, image, media, release, or publication work.
