...
* [ ] Pin the support hardening successor to `c25688a` only after its validator and updater recognize Valve's observed `steamenv_boot linux ...` GRUB entries, preserve that prefix on repeat runs, and use compressed/member limits compatible with the observed 632.5 MB project artifact.
* [ ] Harden the support installer against hostile/corrupt target-root symlinks for every mutation destination (`usr/lib/modules`, firmware, `/etc` policy, Holo database, `/boot`, and persistent state), and require the `/efi` mount to be a distinct expected FAT filesystem before mutation.
* [ ] Bound decompressed module/member sizes and reject every noncanonical extra archive member during both support publication and installation, so an artifact accepted for publication cannot later be rejected or exhaust appliance storage during validation.
* [ ] Validate real Holo package-record contents (including known SteamOS base/kernel records), not only a nonzero count of confined regular `desc` files.
