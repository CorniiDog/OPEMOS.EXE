...
- [ ] Deliver consistent Windows, Linux, and macOS self-contained bundle entry points. The current branch adds a compiled manifest-SHA binding, bundle-relative command resolution without packaged `PATH` fallback, and closed staging validation for the exact platform command inventory, regular-file sizes/hashes, component versions, and declared license files. It also adds `bundle_windows.ps1`, `bundle_linux.sh`, and `bundle_macos.sh` with consistent runtime-root inputs and `dist/<platform>` outputs. Focused Rust and staging regressions pass locally; native Linux and Windows clean-environment package/startup plus representative bundled-command invocation, honest macOS contract validation, exact real payload identities/licenses, broader checks, review, and PR evidence remain pending.
