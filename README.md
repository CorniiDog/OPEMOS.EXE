...
Once the guest reports the exact readiness marker, Rust attaches a selected uncompressed `.img` to QEMU as a read-only virtio block device. Fedora inventories the disk, partitions, filesystems, labels, UUIDs, sizes, and offsets without mounting it. Rust hashes the source before launch and after inspection and fails the build if it changed. Compressed `.img.bz2`, `.img.gz`, and `.img.xz` inputs still require the upcoming normalization stage. NVIDIA build/install logic will come from `open-gpu-kernel-modules-steamos-support`, patched NVIDIA source from `open-gpu-kernel-modules-steamos`, and the compositor payload from `gamescope-nvidia`.
