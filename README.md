...
The raw-device engine is validated with disposable virtual media, while virtual disks remain ineligible in the actual picker. The external-helper protocol binds the exact image, expiring intent, device identity/capacity/nodes, authenticated helper process/version, bounded progress, unmount/open/write/fsync/readback/cancel outcomes, and terminal cleanup. Normal packaged physical-media writing remains disabled before unmount/open until a separately signed macOS privileged helper is installed and independently authenticated/authorized. USB-only mode currently preserves the validated staging image rather than deleting a large output automatically.
