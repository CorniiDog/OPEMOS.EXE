...
The first target is macOS. The desktop shell provides drag-and-drop, file picker fallback, Valve download-page access, and one image-driven build action. Before starting, the user can choose image export, USB export, or both and can select only a whole external physical removable disk. USB selection is non-mutating; after the build, the app rehashes the manifest-bound raw image, revalidates exact device identity and capacity, requires `ERASE diskN` plus a final warning, consumes a short-lived one-use authorization, streams byte progress, reads the written range back for SHA-256 verification, and ejects it. A separate progress window automatically manages the builder appliance, displays live logs and status, and supports cancellation.

The raw-device engine is validated with disposable virtual media, while virtual disks remain ineligible in the actual picker. The cross-platform external-helper protocol binds the exact image, expiring intent, device identity/capacity/nodes, authenticated helper process/version, bounded progress, unmount/open/write/fsync/readback/cancel outcomes, and terminal cleanup. On macOS the application does not run its GUI as root or install a persistent daemon: it invokes Apple's SIP-protected `authopen` for the exact revalidated `/dev/rdiskN` path, receives only that authorized descriptor over `SCM_RIGHTS`, verifies its device identity and read/write flags, and keeps writing, progress, cancellation, read-back verification, and ejection in the bounded Rust engine. Windows still requires a signed UAC helper implementing the same protocol. USB-only mode currently preserves the validated staging image rather than deleting a large output automatically.
...
After input normalization and before QEMU starts, the builder performs a conservative host disk-space preflight. It reserves room for worst-case working-overlay growth, the complete final raw image, export overhead, and 4 GiB of temporary runtime data. Runtime and output volumes are measured separately when appropriate; a shared volume is checked once against the combined requirement. A failure reports the exact required and available byte counts before the expensive guest workflow begins.
...
`b443727ec6a3dd854374e4f4ea997403992353fa`. Its twenty-five required scripts,
...
installer can resolve an independently authenticated offline cache generation;
successful results retain its typed source mode and exact cache identity, while
legacy/direct schema-1 results remain compatible and default to direct input.
The support validator also launches its Python measurement helper through the
