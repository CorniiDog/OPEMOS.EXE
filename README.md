...
The raw-device engine is validated with a small disposable macOS virtual disk, while virtual disks remain ineligible in the actual picker. Normal packaged physical-media writing still requires a signed macOS privileged helper; without it, the backend fails after safe unmount/revalidation with an actionable permission error and ejects the disk. USB-only mode currently preserves the validated staging image rather than deleting a large output automatically.
...
`420688d1ea8c1b0e7a7d16d9a9361d0f3788bf1d`. Its nineteen required scripts,
