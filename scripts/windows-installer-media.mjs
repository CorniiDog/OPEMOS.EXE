...
export async function createWindowsAnswerMedia(root, runner = spawn) {
  ...
  const xml=await readFile(answer,"utf8"), ps=await readFile(provision,"utf8");
  if(!xml.includes("<LogonCount>1</LogonCount>")||!xml.includes("opemos-provision.ps1")||xml.includes("__")) fail("Windows answer file is not the reviewed generated shape.");
  ...
  return {schemaVersion:1,status:"created",filename:path.basename(output),size:Number((await lstat(output,{bigint:true})).size),sha256:await hash(output),inputs:{answerSha256:await hash(answer),provisionSha256:await hash(provision)}};
}
